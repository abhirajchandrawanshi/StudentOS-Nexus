...
def filter_questions(
    *,
    ...
    if company:
        ...
        questions = [question for question in questions if any(item.lower() == company_lower for item in question.get("companies", []))]

    if difficulty:
        difficulty_lower = difficulty.lower()
        questions = [question for question in questions if question.get("difficulty", "").lower() == difficulty_lower]

    if topic:
        topic_lower = topic.lower()
        questions = [question for question in questions if any(item.lower() == topic_lower for item in question.get("topics", []))]
